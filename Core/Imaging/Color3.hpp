...
struct Color3<T>
{
	...
	template<ArithmeticType U> explicit Color3(const tuples::templates::Tuple3<U>& t) noexcept : r(T(t.x)), g(T(t.y)), b(T(t.z)) {}
	explicit Color3(const std::tuple<T, T, T>& t) noexcept : r(std::get<0>(t)), g(std::get<1>(t)), b(std::get<2>(t)) {}
	template<ArithmeticType U> explicit Color3(const std::tuple<U, U, U>& t) noexcept : r(T(std::get<0>(t))), g(T(std::get<1>(t))), b(T(std::get<2>(t))) {}
	explicit Color3(const T* c) noexcept : r(c[0]), g(c[1]), b(c[2]) {}
	template<ArithmeticType U> explicit Color3(const Color3<U>& c) noexcept : r(T(c.r)), g(T(c.g)), b(T(c.b)) {}

	explicit operator tuples::templates::Tuple3<T>() noexcept { return tuples::templates::Tuple3<T>(r, g, b); }
	template<ArithmeticType U> explicit operator tuples::templates::Tuple3<U>() noexcept { return tuples::templates::Tuple3<U>(U(r), U(g), U(b)); }
	//explicit operator std::tuple<T, T, T>() { return std::tuple<T, T, T>(r, g, b); }
	//template<ArithmeticType U> explicit operator std::tuple<U, U, U>() { return std::tuple<U, U, U>(U(r), U(g), U(b)); }
	...
	Color3& operator+=(const Color3& c) noexcept { r += c.r; g += c.g; b += c.b; return *this; }
	Color3& operator-=(const Color3& c) noexcept { r -= c.r; g -= c.g; b -= c.b; return *this; }
	Color3& operator*=(const Color3& c) noexcept { r *= c.r; g *= c.g; b *= c.b; return *this; }
	Color3& operator*=(T f) noexcept { r *= f; g *= f; b *= f; return *this; }
	Color3& operator/=(const Color3& c) noexcept { r /= c.r; g /= c.g; b /= c.b; return *this; }
	Color3& operator/=(T f) noexcept { return operator*=(T(1)/f); }
	...
	template<typename U> static Color3 fromPackedRgb(U c) noexcept;
	std::uint32_t toPackedRgb() const noexcept { return toPackedRgb<std::uint32_t>(); }
	template<typename U> U toPackedRgb() const noexcept;
	template<typename U> static Color3 fromPackedBgr(U c) noexcept;
	std::uint32_t toPackedBgr() const noexcept { return toPackedBgr<std::uint32_t>(); }
	template<typename U> U toPackedBgr() const noexcept;
#if IMAGING_NATIVE_ORDER_RGBA
	template<typename U> static Color3 fromPacked/*Native*/(U c) noexcept { return fromPackedRgb(c); }
	std::uint32_t toPacked/*Native*/() const noexcept { return toPackedRgb(); }
	...
	std::uint32_t toPacked/*Native*/() const noexcept { return toPackedBgr(); }
	...
	bool isFinite() const noexcept { return std::isfinite(r) && std::isfinite(g) && std::isfinite(b); }
	...
	Color3& saturate();
	Color3& makeLinear() noexcept;
	Color3& makeSrgb() noexcept;
...
template<std::floating_point T> const Color3<T> Color3<T>::ZERO{};
template<std::floating_point T> const Color3<T> Color3<T>::UNIT_R{ T(1), T(0), T(0) };
template<std::floating_point T> const Color3<T> Color3<T>::UNIT_G{ T(0), T(1), T(0) };
template<std::floating_point T> const Color3<T> Color3<T>::UNIT_B{ T(0), T(0), T(1) };
template<std::floating_point T> const Color3<T> Color3<T>::ONE{ T(1), T(1), T(1) };
template<std::floating_point T> const Color3<T> Color3<T>::HALF{ T(0.5), T(0.5), T(0.5) };
template<std::floating_point T> const Color3<T> Color3<T>::TOLERANCE{ Constants<T>::TOLERANCE, Constants<T>::TOLERANCE, Constants<T>::TOLERANCE };
template<std::floating_point T> const Color3<T> Color3<T>::INF{ std::numeric_limits<T>::infinity(), std::numeric_limits<T>::infinity(), std::numeric_limits<T>::infinity() };
template<std::floating_point T> const Color3<T> Color3<T>::MINUS_INF{ -std::numeric_limits<T>::infinity(), -std::numeric_limits<T>::infinity(), -std::numeric_limits<T>::infinity() };
template<std::floating_point T> const Color3<T> Color3<T>::LUMINANCE{ T(0.2126), T(0.7152), T(0.0722) };
...
template<std::integral T> const Color3<T> Color3<T>::ZERO{};
...
struct Color3<float>
{
	...
	template<ArithmeticType U> explicit Color3(const tuples::templates::Tuple3<U>& t) noexcept : Color3((float)t.x, (float)t.y, (float)t.z)) {}
	explicit Color3(const std::tuple<float, float, float>& t) noexcept : Color3(std::get<0>(t), std::get<1>(t), std::get<2>(t))) {}
	template<ArithmeticType U> explicit Color3(const std::tuple<U, U, U>& t) noexcept : Color3((float)std::get<0>(t), (float)std::get<1>(t), (float)std::get<2>(t))) {}
	...
	template<ArithmeticType U> explicit Color3(const tuples::templates::Tuple3<U>& t) noexcept : rgb(simd::set3((float)t.x, (float)t.y, (float)t.z)) {}
	explicit Color3(const std::tuple<float, float, float>& t) noexcept : rgb(simd::set3(std::get<0>(t), std::get<1>(t), std::get<2>(t))) {}
	template<ArithmeticType U> explicit Color3(const std::tuple<U, U, U>& t) noexcept : rgb(simd::set3((float)std::get<0>(t), (float)std::get<1>(t), (float)std::get<2>(t))) {}
	explicit Color3(const float* c) noexcept : rgb(simd::load3(c)) {}
#endif
	explicit Color3(simd::float4 c) noexcept : rgb(c) {}
	Color3(const Color3& c) noexcept : rgb(c.rgb) {}
	template<ArithmeticType U> explicit Color3(const Color3<U>& c) noexcept : Color3((float)c.r, (float)c.g, (float)c.b) {}
...
#else
	Color3& operator/=(const Color3& c) noexcept { rgb = simd::div3(rgb, c); return *this; }
#endif
	...
	template<> simd::float4& get() noexcept { return rgb; }
	template<> const simd::float4& get() const noexcept { return rgb; }
...
const Color3<float> Color3<float>::HALF{ 0.5f, 0.5f, 0.5f };
...
template<typename T>
	requires (std::floating_point<T> || std::integral<T>)
inline Color3<T> operator*(T f, const Color3<T>& c) noexcept
{ 
	return Color3<T>(f*c.r, f*c.g, f*c.b); 
}

template<typename T>
	requires (std::floating_point<T> || std::integral<T>)
inline Color3<T> operator*(const Color3<T>& c, T f) noexcept
{ 
	return Color3<T>(c.r*f, c.g*f, c.b*f); 
}
...
inline std::basic_istream<C, T>& operator>>(std::basic_istream<C, T>& s, Color3<U>& c)
{ 
	return s >> c.r >> std::ws >> c.g >> std::ws >> c.b; 
}

template<typename C, typename T, typename U>
	requires std::floating_point<U> || std::integral<U>
inline std::basic_ostream<C, T>& operator<<(std::basic_ostream<C, T>& s, const Color3<U>& c)
{ 
	constexpr C WS(0x20);
	return s << c.r << WS << c.g << WS << c.b;
}
...
{
#if IMAGING_SIMD_EXPAND_LAST
	return Color3<float>(simd::div4(c1, c2));
#else
	return Color3<float>(simd::div3(c1, c2));
#endif
}
...
{ 
#if IMAGING_SIMD_EXPAND_LAST
	return Color3<float>(simd::div4(simd::set4(f), c));
#else
	return Color3<float>(simd::div3(simd::set3(f), c));
#endif
}
...
inline Color3<float> operator/(const Color3<float>& c, float f) noexcept 
{ 
	return Color3<float>(simd::div4(c, simd::set4(f))); 
}

template<typename C, typename T>
inline std::basic_istream<C, T>& operator>>(std::basic_istream<C, T>& s, Color3<float>& c)
{ 
	float r, g, b; 
	s >> r >> std::ws >> g >> std::ws >> b; 
	c.set(r, g, b); 
	return s; 
}
...
template<typename T>
	...
	requires std::floating_point<T>
#endif
inline Color3<T> saturate(Color3<T>&& c)
{
...
template<typename T>
	...
	requires std::floating_point<T>
#endif
inline Color3<T> makeLinear(Color3<T>&& c) noexcept
{
	c.r = makeLinear(c.r);
	c.g = makeLinear(c.g);
	c.b = makeLinear(c.b);
...
{
	return Color3<T>(makeSrgb(c.r), makeSrgb(c.g), makeSrgb(c.b));
}
...
template<typename T>
	...
	requires std::floating_point<T>
#endif
inline Color3<T> makeSrgb(Color3<T>&& c) noexcept
{
	c.r = makeSrgb(c.r);
	c.g = makeSrgb(c.g);
	c.b = makeSrgb(c.b);
...
template<size_t I, typename T>
struct tuple_element<I, ::core::imaging::templates::Color3<T>>
{
	using type = T;
};
...
template<typename T>
struct tuple_size<::core::imaging::templates::Color3<T>> : integral_constant<size_t, 3> 
{
};
...
template<typename T>
struct hash<::core::imaging::templates::Color3<T>>
{
	std::size_t operator()(const ::core::imaging::templates::Color3<T>& c) const noexcept
	{
		std::hash<T> hasher;
