...
struct Color3<T>
{
	...
	bool approxEquals(const Color3& c) const noexcept;
	bool approxEquals(const Color3& c, T tolerance) const noexcept;
...
struct Color3<float>
{
	...
	template<Arithmetic U> explicit Color3(const Color3<U>& c) noexcept : Color3((float)c.r, (float)c.g, (float)c.b) {}
...
template<typename T>
	requires (std::floating_point<T> || std::integral<T>)
inline Color3<T> min(const Color3<T>& c1, const Color3<T>& c2)
{
...
template<typename T>
	requires (std::floating_point<T> || std::integral<T>)
inline Color3<T> max(const Color3<T>& c1, const Color3<T>& c2)
{
